...
public class weatherServiceImpl implements weatherService {
    ...
    private void setHourlyWeatherParameters() {
        try {
            ...
            if(!obj.has("list")) {
                System.out.println("No 'List' has found in the JSON. ");
                return;
            }
            ...
            for(int i = 0; i < obj.getJSONArray("list").length(); i++) {
                ...
                String time = null;
                if (obj.getJSONArray("list").getJSONObject(i).has("main")) {
                    time = obj.getJSONArray("list").getJSONObject(i).getString("dt_txt").split(" ")[1];
